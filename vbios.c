...
typedef struct {
    ...
    ULONG VBIOSImageOffset;        // 0x34
    ULONG Lib1ImageOffset;         // 0x38
...
int main(int argc, char** argv) {
    ...
    if (tbl_size != fread(pvfct, 1, tbl_size, fp_vfct)) {
        fprintf(stderr, "%s: failed to read VFCT table!\n", argv[0]);
        return -1;
    }
    ...
    while (offset < tbl_size) {
        ...
        printf("dump vbios to %s successfully\n", vbios_name);
